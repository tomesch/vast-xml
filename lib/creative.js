...
function Creative(type, settings) {
  ...
  if (type == 'Linear' && settings.skipoffset) this.skipoffset = settings.skipoffset;
  ...
  this.trackingEvents = [];
  this.attachTrackingEvent = function(type, url, offset) {
    this.trackingEvents.push(new TrackingEvent(type, url, offset));
    return this;
  };
  this.videoClicks = [];
